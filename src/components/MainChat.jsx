...
function MainChat({ conversationId }) {
  ...
  // Voice call functionality - disabled until backend is set up
  // const {
  //   callState,
  ...
  if (!conversationId) {
    return (
      <main className="chat-main flex-1 bg-[#111827] flex items-center justify-center">
  ...
  if (!isGroupChat && otherUser && isBlocked) {
    return (
      <main className="chat-main flex-1 bg-[#111827] flex flex-col">
        {/* Chat Header */}
  ...
  return (
    <main className="chat-main flex-1 flex flex-col transition-colors duration-300" style={{ backgroundColor: 'var(--background)' }}>
      {/* Chat Header */}
      ...
      {/* Voice Call Modal - Disabled until backend is set up */}
      {/* {callState && (
        <VoiceCallModal
          callState={callState}
          otherUser={callOtherUser}
